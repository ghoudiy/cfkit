...
from cfkit.util.common import extensions, language_conf, read_json_file, language_conf_path
from cfkit.config import set_language_attributes


def execute_file(file: Path, input_path: Path, output_path: str, memory_limit: float, *args) -> None:
  '''
  ...
  programming_language = extensions.get(file.suffix)
  ...
  run_command = run_command.replace("%%{input_file}%%", str(input_path))
  run_command = run_command.replace("%%{output_file}%%", output_path)
  run_command = run_command.replace("%%{output}%%", output)
  run_command = run_command.replace("%%{dir_name}%%", str(input_path.parent))
